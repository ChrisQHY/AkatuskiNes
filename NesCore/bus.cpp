...
void Bus::clock()
{
    ...
    if(cartridge.mapperPtr->irqState()) {
        cartridge.mapperPtr->irqClear();
        Cpu.irq();
    }
...
{
    stream << bus.cartridge.md5;
    stream << bus.Cpu;
    stream << bus.Ppu;
    stream << bus.Apu;
    bus.cartridge.mapperPtr->writeToStream(stream);
...
QDataStream &operator>>(QDataStream &stream, Bus &bus)
{
    ...
    stream >> bus.Ppu;
    stream >> bus.Apu;
    bus.cartridge.mapperPtr->readFromStream(stream);
