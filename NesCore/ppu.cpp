...
void PPU::reset()
{
    ...
    odd_frame = false;
    memset(frameData, 0, sizeof(QRgb) * 256 * 240);
...
quint8 PPU::ppuRead(quint16 addr)
{
    ...
    if(addr <= 0x1FFF) {
        ...
        switch(cart->mapperPtr->nametableMirror) {
...
void PPU::clock()
{
    ...
    if(x >= 0 && x < 256 && y >= 0 && y < 240) {
        ...
        frameData[y * 256 + x] = qRgb(RGBColorMap[palette_addr][0],
                                      RGBColorMap[palette_addr][1],
                                      RGBColorMap[palette_addr][2]);
    }
